---
title: "Preliminary analysis Survey 1"
...
```{r Load data}
load('data/Survey_1.rda')
# Dataframe is called VV, contains eligible responses and survey weights.

#VV$weight = 1 # Unweighted
VV$weight = VV$Weights # Weighted
...
```{r age}
g <- ggplot(data=VV, aes(x=Q3, weight=weight)) +
...
g <- ggplot(data=VV, aes(x = Q3, y = Q4, weight=weight)) +
...
```{r Gender}
g1 <- ggplot(data=VV, aes(x=Q4, weight=weight)) +
...
g2 <- ggplot(data=VV, aes(x=Q4, weight=1)) +
  geom_bar(fill='yellow',colour='black') +
  xlab('Gender identity') + ylab('Count') +
  ggtitle('Self reported gender identity', subtitle='(Unweighted)') +
  theme_classic(base_size = 25)  +
  theme(axis.text.x = element_text(angle=-45, hjust=0))
g <- ggpubr::ggarrange(g1,g2,nrow=1)
g
...
g1 <- ggplot(data=VV , aes(x=Q7a, weight=weight)) +
  geom_bar(fill='yellow',colour='black') +
...
g2 <- ggplot(data=VV , aes(x=Q7a)) +
  geom_bar(fill='yellow',colour='black') +
  xlab('Education level') + ylab('Count') +
  ggtitle('Highest level of education', subtitle = 'Unweighted') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-70, hjust=0))
g <- ggpubr::ggarrange(g1,g2,nrow=1)
g
...
Respondent are well educated, with 31,000 having a primary degree, and 34,00 a post-graduate degree. Again, this is quite differnet form the underlying population, as shown by the wieghted data.

## Q8 Covid pandemic unemployment

```{r CovidUnemployment}
kable(table(VV$Q8))
kable(questionr::wtd.table(VV$Q8, weights= VV$weight,
                     digits=0,normwt=TRUE))

kable(100*round(prop.table(table(VV$Q8, VV$Q7b),2),3))
q <- questionr::wtd.table(VV$Q8, VV$Q7b, weights= VV$weight,
                     digits=0,normwt=TRUE)
kable(q)
kable(questionr::prop(q))

g1 <- ggplot(data=VV , aes(x=Q8, weight = weight)) +
  geom_bar(fill='yellow',colour='black') +
  xlab('Covid Unemployment payment') + ylab('Count') +
  ggtitle('Receipt of Covid19 Unemployment support', subtitle = 'Weighted') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-45, hjust=0))
g2 <- ggplot(data=VV , aes(x=Q8, weight = 1)) +
  geom_bar(fill='yellow',colour='black') +
  xlab('Covid Unemployment payment') + ylab('Count') +
  ggtitle('Receipt of Covid19 Unemployment support', subtitle = 'Unweighted') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-45, hjust=0))
g <- ggpubr::ggarrange(g1,g2,nrow=1)
g
...
kable(questionr::wtd.table(VV$Q9a, weights= VV$weight,
                     digits=0,normwt=TRUE))
prop.table(questionr::wtd.table(VV$Q9a, weights= VV$weight,
                     digits=0,normwt=TRUE))

kable(100*round(prop.table(table(VV$Q9a, VV$Q7b),2),3))
q <- questionr::wtd.table(VV$Q9a, VV$Q7b, weights= VV$weight,
                     digits=0,normwt=TRUE)
kable(q)
kable(questionr::prop(q))

g1 <- ggplot(data=VV , aes(x=Q9a, weight=weight)) +
  geom_bar(fill='yellow',colour='black') +
  xlab('Living arrangements') + ylab('Count') +
  ggtitle('Do you live alone or with others?', subtitle = 'Weighted') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-45, hjust=0))
g2 <- ggplot(data=VV , aes(x=Q9a, weight=1)) +
  geom_bar(fill='yellow',colour='black') +
  xlab('Living arrangements') + ylab('Count') +
  ggtitle('Do you live alone or with others?', subtitle = 'Unweighted') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-45, hjust=0))
g <- ggpubr::ggarrange(g1,g2,nrow=1)
g
...
g1 <- ggplot(data=VV , aes(x=Q11a, weight=weight)) +
  geom_bar(fill='yellow',colour='black') +
...
g2 <- ggplot(data=VV , aes(x=Q11a, weight=1)) +
  geom_bar(fill='yellow',colour='black') +
  xlab('Number of people') + ylab('Count') +
  ggtitle('People outside your household met face-to-face', subtitle = 'Unweighted') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-45, hjust=0))
g <- ggpubr::ggarrange(g1,g2,nrow=1)
g
...
# Reverse some items
MOS_Reverse <- function (df, item) {
  Item <- enquo(item)
  
  df <- df %>%
    mutate(paste0(item,'b') = !!Item*1000)
}
