...
The large majority of respondents have access to broadband internet, but small number used a dialup connection, and many used their mobile phone.
...
VV_2 %>% select(Q95) %>%
  ...
  group_by(Q95) %>%
  summarise(N = n()) %>%
  mutate(Pct = round(100*N/sum(N),2)) %>% kable(caption = "Have you extra caring responsibilites now?")
...
Far fewer were waiting for a test. Most of those tested were waiting, either for a test, or for the results of a test. Of the 207 test results that had come back 37% were positive, slightly higehr than two weeks ago.
...
g <- ggplot(Lock_Down, aes(y=Measure,x = Rank)) +
  geom_boxplot(fill='yellow', colour='black') +
  ggtitle('Preferences for moving out of lockdown') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-90, hjust=0))
...
g <- ggplot(Lock_Down, aes(y=Measure,x = Rank, )) +
  geom_boxplot(fill='yellow',colour='black') +
  facet_wrap(~Sex) +
  ggtitle('Preferences for moving out of lockdown', subtitle='by Sex') +
  theme_classic(base_size = 25) +
  theme(axis.text.x = element_text(angle=-90, hjust=0))
g
...
g <- plot_model(model)
g <- g +
  ggtitle('Preferences for moving out of lockdown', subtitle='Effect on ranking') +
