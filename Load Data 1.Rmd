---
title: "Load and save data from the first survey"
...
VV_1.names <- read_csv('data/Survey1/Corona Citizens Science Project V0.3_April 9, 2020_02.29.csv', n_max=1) # Final Survey 1
VV_1 <- read_csv('data/Survey1/Corona Citizens Science Project V0.3_April 9, 2020_02.29.csv', guess_max = 10000, skip=3, col_names=names(VV_1.names)) %>%
  mutate(ID=seq_along(StartDate)) %>%
  mutate(ID = as.character(ID))

NN.names <- read_csv('data/Survey1/Corona Citizens Science Project V0.3_April 9, 2020_02.34.csv', n_max=1)
NN <- read_csv('data/Survey1/Corona Citizens Science Project V0.3_April 9, 2020_02.34.csv', guess_max = 10000, skip=3, col_names=names(NN.names)) %>%
  mutate(ID=seq_along(StartDate)) %>%
  mutate(ID = as.character(ID))


rm(VV_1.names,NN.names)
```

#Remove Spam and Preview survey answers

There are 83 answers flagged as Spam, and 80 as Survey preview, with 105761 valid answers left.

```{r Remove Spam and Preview answers}
VV_1 <- VV_1 %>%
  filter(Status == 'IP Address') # 105761
NN <- NN %>%
  filter(Status == 0) # 105763

Extras <- NN %>%
  anti_join(VV_1, by='ResponseId') # Two late responses in NN, but not in VV_1
...
kable(table(VV_1$Q1),booktabs = TRUE) %>% kable_styling(latex_options = 'HOLD_position')
...
kable(table(VV_1$Q66),booktabs = TRUE) %>% kable_styling(latex_options = 'HOLD_position')
...
VV_1 <- VV_1 %>%
  ...
  filter(Q3 >= 15) # 103845 - lose 206 younger people
```

There are 103,824 valid responses.
...
Weights_Survey <- VV_1 %>% select(ID, Q3,Q4, Q5,Q7) %>%
  rename(Sex = Q4, Age = Q3, County = Q5, Education = Q7) # Matching variables in the Survey data
...
VV_1 %>% distinct(Age_group)
VV_1 %>% distinct(Age_group_census)
table(VV_1$Age_group)
```
...
# Then recode the survey data to match the revised census data
VV_1 <- VV_1 %>%
  mutate(County_merge = recode(Q5, 'Carlow' = 'Carlow',
                  'Cavan' = 'Cavan','Clare' = 'Clare','Cork city' = 'Cork City',
                  'Donegal' = 'Donegal','Dublin 1' = 'Dublin','Dublin 10' = 'Dublin',
                  'Dublin 11' = 'Dublin','Dublin 12' = 'Dublin','Dublin 13' = 'Dublin',
                  'Dublin 14' = 'Dublin','Dublin 15' = 'Dublin','Dublin 16' = 'Dublin',
...
table(VV_1$Q5,VV_1$County_merge)
```
...
Weights_CSO <- Weights_CSO %>%
  mutate(Education_Survey = recode(Education,
                ...
                "Honours bachelor degree/professional qualification or both" = "University degree",
                ...
                "Postgraduate diploma or degree" = "Postgraduate",
...
VV_1 <- bind_rows(VV_1_MF, VV_1_Other)


rm(Weights_Other, Weights_MF, VV_1_Other, VV_1_MF)
```
...
#County
ggplot(data=VV_1, aes(x = Weights, y = County, colour=Sex, group = interaction(Sex,  County))) + geom_boxplot()
ggplot(data=VV_1, aes(x = Weights_trimmed, y = County, colour=Sex, group = interaction(Sex,  County))) + geom_boxplot()

#Education
ggplot(data=VV_1, aes(x = Weights, y = Education, colour=Sex, group = interaction(Sex,  Education))) + geom_boxplot()
ggplot(data=VV_1, aes(x = Weights_trimmed, y = Education, colour=Sex, group = interaction(Sex,  Education))) + geom_boxplot()

#Age
ggplot(data=VV_1, aes(x = Weights, y = Age_group, colour=Sex, group = interaction(Sex,  Age_group))) + geom_boxplot()
ggplot(data=VV_1, aes(x = Weights_trimmed, y = Age_group, colour=Sex, group = interaction(Sex,  Age_group))) + geom_boxplot()
...
save(VV_1,file = 'data/Survey1/Survey_1.rda')
write_csv(VV_1,'data/Survey1/Survey_1.csv')
