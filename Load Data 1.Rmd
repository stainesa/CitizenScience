...
Extras <- NN %>%
  anti_join(VV, by='ResponseId') # Two late responses in NN, but not in VV
...
Weights_Survey <- VV %>% select(ID, Q3,Q4, Q5,Q7) %>%
...
VV %>% distinct(Age_group)
VV %>% distinct(Age_group_census)
table(VV$Age_group)
```
...
Weights_Other = Weights_CSO_matched %>%
  ...
  select(Education_Survey, Age_group, County, Sex, Weights) %>%
...
VV <- bind_rows(VV_MF, VV_Other)


rm(Weights_Other, Weights_MF, VV_Other, VV_MF)
```

Finally we save the survey one data frame.

```{r save file}

save(VV,file = 'data/Survey_1.rda')
write_csv(VV,'data/Survey_1.csv')
